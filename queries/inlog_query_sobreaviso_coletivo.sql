/*
🎯 QUERY CORRIGIDA PARA RELATÓRIO INDIVIDUAL DE SOBREAVISO
CORRIGE: Separação correta entre trabalho efetivo e sobreaviso
...
-- CALCULAR SOBREAVISO E TRABALHO EFETIVO SEPARADAMENTE
sobreaviso_calculado AS (
    SELECT 
        ...
        -- CALCULAR HORAS TRABALHADAS NO SOBREAVISO (apenas o tempo efetivo trabalhado das 19:00 em diante)
        CASE 
            WHEN d.entrada_3 IS NOT NULL AND d.saida_3 IS NOT NULL THEN
                -- Verificar se as batidas estão dentro da janela de sobreaviso
                CASE 
                    WHEN EXTRACT(DOW FROM d.date) BETWEEN 1 AND 5 THEN -- Seg-Sex: 19:00-22:00
                        CASE 
                            WHEN date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i') >= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 19:00', '%Y-%m-%d %H:%i') THEN
                                date_diff('minute', 
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i'),
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i')
                                )
                            ELSE 0
                        END
                    WHEN EXTRACT(DOW FROM d.date) IN (0, 6) THEN -- Dom-Sab: 09:00-21:00
                        CASE 
                            WHEN date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i') >= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 09:00', '%Y-%m-%d %H:%i') THEN
                                date_diff('minute', 
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i'),
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i')
                                )
                ...
                END
            ELSE 0
        END AS horas_trabalhadas_sobreaviso,
        
        -- CALCULAR TEMPO DE SOBREAVISO (disponibilidade total - trabalho efetivo)
        CASE 
            WHEN EXTRACT(DOW FROM d.date) BETWEEN 1 AND 5 THEN -- Seg-Sex: 19:00-22:00 (3 horas = 180 min)
                CASE 
                    WHEN d.entrada_3 IS NOT NULL AND d.saida_3 IS NOT NULL THEN
                        -- Verificar se as batidas estão dentro da janela de sobreaviso (19:00-22:00)
                        CASE 
                            WHEN date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i') >= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 19:00', '%Y-%m-%d %H:%i') AND
                                 date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i') <= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 22:00', '%Y-%m-%d %H:%i') THEN
                                -- Tempo total de sobreaviso (3h) - tempo trabalhado
                                180 - date_diff('minute', 
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i'),
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i')
                                )
                            ELSE 180 -- Se não está na janela, tempo total de sobreaviso
                        END
                    ELSE 180 -- Se não tem batidas, tempo total de sobreaviso
                END
            WHEN EXTRACT(DOW FROM d.date) IN (0, 6) THEN -- Dom-Sab: 09:00-21:00 (12 horas = 720 min)
                CASE 
                    WHEN d.entrada_3 IS NOT NULL AND d.saida_3 IS NOT NULL THEN
                        -- Verificar se as batidas estão dentro da janela de sobreaviso (09:00-21:00)
                        CASE 
                            WHEN date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i') >= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 09:00', '%Y-%m-%d %H:%i') AND
                                 date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i') <= 
                                 date_parse(CAST(d.date AS VARCHAR) || ' 21:00', '%Y-%m-%d %H:%i') THEN
                                -- Tempo total de sobreaviso (12h) - tempo trabalhado
                                720 - date_diff('minute', 
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.entrada_3, '%Y-%m-%d %H:%i'),
                                    date_parse(CAST(d.date AS VARCHAR) || ' ' || d.saida_3, '%Y-%m-%d %H:%i')
                                )
                            ELSE 720 -- Se não está na janela, tempo total de sobreaviso
                        END
                    ELSE 720 -- Se não tem batidas, tempo total de sobreaviso
                END
            ELSE 0
        END AS tempo_sobreaviso_minutos
...
SELECT 
    ...
    -- HORAS REGULARES (apenas jornada normal, máximo 8 horas)
    CASE 
        WHEN sc.trabalhados_minutos > 0 THEN
            CONCAT(
                LPAD(CAST(FLOOR(LEAST(sc.trabalhados_minutos, 480) / 60) AS VARCHAR), 2, '0'), ':',
                LPAD(CAST(LEAST(sc.trabalhados_minutos, 480) % 60 AS VARCHAR), 2, '0')
            )
        ELSE '00:00'
    END AS horas_regulares,
    
    -- HORAS DE SOBREAVISO (regra de negócio)
    CONCAT(
    ...
    -- TEMPO TOTAL DE PRESENÇA (apenas trabalho efetivo, excluindo sobreaviso)
    CONCAT(
        LPAD(CAST(FLOOR((sc.trabalhados_minutos - sc.horas_trabalhadas_sobreaviso) / 60) AS VARCHAR), 2, '0'), ':',
        LPAD(CAST((sc.trabalhados_minutos - sc.horas_trabalhadas_sobreaviso) % 60 AS VARCHAR), 2, '0')
    ) AS tempo_total_presenca,
    
    -- HORAS EXTRAS (apenas das 17:00 às 19:00, máximo 2 horas)
    -- Excluir o tempo trabalhado no sobreaviso (19:00 em diante)
    CASE 
        WHEN sc.trabalhados_minutos > 480 THEN
            -- Calcular apenas o tempo trabalhado das 17:00 às 19:00
            -- Subtrair o tempo trabalhado no sobreaviso para não contar como hora extra normal
            CONCAT(
                LPAD(CAST(FLOOR(LEAST(
                    GREATEST(0, sc.trabalhados_minutos - 480 - sc.horas_trabalhadas_sobreaviso), 
                    120
                ) / 60) AS VARCHAR), 2, '0'), ':',
                LPAD(CAST(LEAST(
                    GREATEST(0, sc.trabalhados_minutos - 480 - sc.horas_trabalhadas_sobreaviso), 
    ...
    -- HORAS TRABALHADAS NO SOBREAVISO (tempo efetivo trabalhado)
    CONCAT(
        LPAD(CAST(FLOOR(sc.horas_trabalhadas_sobreaviso / 60) AS VARCHAR), 2, '0'), ':',
        LPAD(CAST(sc.horas_trabalhadas_sobreaviso % 60 AS VARCHAR), 2, '0')
    ) AS hrs_trabalhadas_sobreaviso,
    
    -- TEMPO DE SOBREAVISO (disponibilidade total - trabalho efetivo)
    CONCAT(
        LPAD(CAST(FLOOR(sc.tempo_sobreaviso_minutos / 60) AS VARCHAR), 2, '0'), ':',
        LPAD(CAST(sc.tempo_sobreaviso_minutos % 60 AS VARCHAR), 2, '0')
    ) AS tempo_sobreaviso,
    
    -- HORAS NEGATIVAS (diferença entre jornada padrão de 8h e horas regulares trabalhadas)
    -- Calcular apenas com horas regulares (excluindo sobreaviso)
    CASE 
        WHEN (sc.trabalhados_minutos - sc.horas_trabalhadas_sobreaviso) < 480 THEN
            CONCAT(
                '-',
                LPAD(CAST(FLOOR((480 - (sc.trabalhados_minutos - sc.horas_trabalhadas_sobreaviso)) / 60) AS VARCHAR), 2, '0'), ':',
                LPAD(CAST((480 - (sc.trabalhados_minutos - sc.horas_trabalhadas_sobreaviso)) % 60 AS VARCHAR), 2, '0')
            )
        ELSE '00:00'
    ...
    CONCAT(
        ...
        END,
        'Tempo total sobreaviso = ',
        CONCAT(
            LPAD(CAST(FLOOR(sc.tempo_sobreaviso_minutos / 60) AS VARCHAR), 2, '0'), ':',
            LPAD(CAST(sc.tempo_sobreaviso_minutos % 60 AS VARCHAR), 2, '0')
        ),
        ', Tempo total hora extra = ',
        CASE 
            WHEN sc.trabalhados_minutos > 480 THEN
                CONCAT(
                    LPAD(CAST(FLOOR(LEAST(
