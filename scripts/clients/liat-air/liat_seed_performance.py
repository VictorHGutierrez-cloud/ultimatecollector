...
def build_liat_questionnaire(catalog: dict, phase_key: str) -> List[dict]:
    """Questionário BSC: 4 perspectivas, 360° (P2/P3), coaching/IDP.

    Competências vêm do bloco nativo 'Competencies assessment' da Factorial
    (competencies_assessments_enabled=true) — não duplicar perguntas manuais.
    """
        ...
        objective_questions.append(
            {
                ...
                "title": (
                    ...
                    f"Weight: 70% of final rating. Scale: {'; '.join(r['code'] for r in catalog['rating_scale'])}."
                ),
                "answer_type": "rating",
            }
        )
    objective_questions.append(
        {
            ...
            "with_comment": True,
            "title": (
                "List or confirm SMART goals cascaded from the corporate BSC scorecard "
                "(include measure, owner, due date, and parent corporate objective link)."
            ),
            "answer_type": "text",
        }
    )
        ...
        objective_questions.append(
            {
                ...
                "mandatory": True,
                "with_comment": True,
                "title": "Mid-year progress vs BSC measures, challenges, and approved goal adjustments.",
                "answer_type": "text",
            }
        )
    if phase_key == "p3":
        objective_questions.append(
            {
                ...
                "mandatory": True,
                "with_comment": True,
                "title": "Year-end evidence summary vs agreed BSC measures and next-year development focus.",
                "answer_type": "text",
            }
        )

    peer_questions = []
    if phase_key in ("p2", "p3"):
        peer_questions = [
            {
                "uuid": new_uuid(),
                "mandatory": True,
                "with_comment": True,
                "title": "Peer feedback: collaboration, safety culture, and teamwork observations.",
                "answer_type": "rating",
            },
            {
                "uuid": new_uuid(),
                "mandatory": False,
                "with_comment": True,
                "title": "Peer comments (anonymised where policy requires).",
                "answer_type": "text",
            },
        ]
    ...
    coaching_questions = [
        ...
        },
        {
            ...
            "mandatory": True,
            "with_comment": True,
            "title": "Individual Development Plan (IDP): actions linked to BSC goals and 9-box placement.",
            "answer_type": "text",
        },
        {
            "uuid": new_uuid(),
            "mandatory": False,
            "with_comment": True,
            "title": "Named coach for IDP follow-up (minimum semi-annual review).",
            "answer_type": "text",
        },
    ]
    if phase_key == "p3":
        coaching_questions.append(
            {
                ...
                "mandatory": True,
                "with_comment": True,
                "title": "Potential assessment input for 9-box talent matrix (performance vs potential).",
                "answer_type": "rating",
            }
        )
    ...
    sections = [
        {
            "uuid": new_uuid(),
            "type": "section",
            "section_title": "LIAT BSC Objectives (70%)",
        ...
        sections.append(
            {
                "uuid": new_uuid(),
                "type": "section",
                "section_title": "LIAT 360° Feedback",
        ...
        [
            {
                "uuid": new_uuid(),
                "type": "section",
                "section_title": "LIAT Coaching & IDP",
                ...
                "questions": [
                    {
                        ...
                        "mandatory": False,
                        "with_comment": False,
                        "title": "LIAT rating reference (informational)",
...
class LiatSeeder:
    ...
    def ensure_location(self) -> Optional[str]:
        ...
        if not existing:
            # Prefer renaming a non-LIAT office if only generic locations exist
            for loc in locations:
                name = str(loc.get("name") or "")
                if "LIAT" not in name and "VC Bird" not in name:
                    existing = loc
                    break
        ...
        if existing and self.apply:
            # Update existing generic office into LIAT Antigua location
    ...
    def ensure_teams(self, cast: Dict[str, dict]) -> Dict[str, str]:
        ...
        for team in self.catalog["teams"]:
            ...
            payload = {
                "name": team["name"],
                "description": f"LIAT demo team for BSC performance narrative ({team['key']})",
            }
            if not self.apply:
    ...
    def _apply_questionnaire(
        ...
        questionnaire = build_liat_questionnaire(self.catalog, process_def["key"])
    ...
    def ensure_review_processes(self, cast: Dict[str, dict]) -> Dict[str, dict]:
        author = (
            cast.get("Denise Antoine")
            or cast.get("Marcus Williams")
    ...
    def ensure_trainings(self, cast: Dict[str, dict]) -> None:
        ...
        author = cast.get("Denise Antoine") or cast.get("Marcus Williams") or next(iter(cast.values()))
        created_ids = []
    ...
    def _publish_post(self, post_id: str, post: dict, group_id: str) -> None:
        """Posts criados via API ficam em rascunho até published_at ser definido."""
    ...
    def write_goals_pack(self, cast: Dict[str, dict]) -> Path:
        lines = [
            "# LIAT BSC SMART Goals Pack",
            "",
            "Use these goals when completing Q1 agreements / BSC objective text answers in Factorial.",
            "",
            f"Company: {self.catalog.get('company_id')} · LIAT AIR demo cast (Antigua)",
            "",
        ]
        for person in cast.values():
    ...
    def run(self) -> int:
        print(f"=== LIAT AIR Seed ({'APPLY' if self.apply else 'DRY-RUN'}) ===")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed LIAT AIR demo (BSC performance)")
    ...
    return LiatSeeder(apply=args.apply).run()
