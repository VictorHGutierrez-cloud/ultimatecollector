#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Verify STATIN PMS demo seed is ready (company 191864)."""
...
def main() -> int:
    ...
    print("=== STATIN PMS Verify Demo ===")
    print(f"Company focus: {catalog.get('company_id')} · {catalog.get('display_name')}")
    ...
    for person in catalog["cast"]:
        ...
        print(
            ...
            f"id={emp.get('id')} band={person.get('demo_band')} manager={emp.get('manager_id')}"
        )
    ...
    for process_def in catalog["review_processes"]:
        ...
        mark = "OK" if coverage >= max(1, len(cast_access) - 1) else "WARN"
        print(
        ...
        if stq == 200:
            raw = str(bodyq)
            has_statin = "STATIN" in raw or "GoJ" in raw or "Individual Work Plan" in raw
            print(f"    questionnaire_statin_wording={'OK' if has_statin else 'WARN'}")
            if not has_statin:
                ok = False
    ...
    goals = RUN_LOG_DIR / "goals_pack.md"
    print(f"\nGoals pack: {'OK' if goals.exists() else 'FAIL'} {goals}")
    if not goals.exists():
        ok = False
